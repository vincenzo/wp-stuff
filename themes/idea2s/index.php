<?php get_header(); ?>
	<div class="narrowcolumn">
		...
		<div class="post" id="post-<?php the_ID(); ?>">
			...
			<div class="postinfo">
			    ...
			    <?php 
                    ...
                    if (($post_counter <= 5) and (!is_paged())) {
                        the_content(__('Read more' , 'idea_domain') . " &raquo;");
                    }
                    else {
                        if (function_exists('the_thumb')) the_thumb(); 
                        the_excerpt();
                        echo '<a class="more-link" href="'. get_permalink() . '#more-' . get_the_ID() .'">' . __('Read more', 'idea_domain') . ' &raquo; </a>';
                    }
                ?>
			</div>
			<div class="postinfo">
		...
		<div class="clear"></div>
		<?php if (($post_counter <= 5) and (!is_paged())) { ?>
		<div class="authorinfo">
    		<p class="categories"><?php _e('Categories', 'idea_domain'); ?>: <?php the_category(', ') ?></p>
