...
function fdfootnote_conf() {
  ...
  if (!isset($options['fdfootnote_collapse'])) {
...
<div class="wrap">
  ...
  <form action="" method="post" id="fdfootnote-conf">
    ...
    <p>
      <input id="fdfootnote_single" name="fdfootnote_single" type="checkbox"
             value="1"<?php if ($options['fdfootnote_single'] == 1) {
        echo ' checked';
      } ?> />
      <label
          for="fdfootnote_single"><?php _e('Only show footnotes on single post/page', FDFOOTNOTE_TEXTDOMAIN); ?></label>
    </p>

    <p>
      <input id="fdfootnote_collapse" name="fdfootnote_collapse" type="checkbox"
             value="1"<?php if ($options['fdfootnote_collapse'] == 1) {
        echo ' checked';
      } ?> />
      <label for="fdfootnote_collapse"><?php _e('Collapse footnotes until clicked', FDFOOTNOTE_TEXTDOMAIN); ?></label>
    </p>

    <p class="submit" style="text-align: left"><?php wp_nonce_field('fdfootnote', 'fdfootnote-admin'); ?><input
        type="submit" name="submit" value="<?php _e('Save', FDFOOTNOTE_TEXTDOMAIN); ?> &raquo;"/></p>
  </form>
</div>
<?php
}
...
function fdfootnote_convert($content) {
  ...
  $linksingle = false;
  if (isset($options['fdfootnote_collapse'])) {
    $collapse = $options['fdfootnote_collapse'];
  }
  if (isset($options['fdfootnote_single'])) {
    $single = $options['fdfootnote_single'];
  }
  ...
  if (preg_match_all('/\[(\d+\. .*?)\]/s', $content, $matches)) {
    ...
    if (!$linksingle) {
      ...
      $notes = _assoc_array_flip($notes);
      foreach ($notes as $key => $value) {
        $content .= "<li>";
        foreach ($value as $v) {
          $content .= "<sup class='footnote'><a id='fn-$post_id-$v' href='#fnref-$post_id-$v'>$v</a>&nbsp;&nbsp;</sup>";
        }
        $content .= "$key</li>";
      }
      $content .= "</ul>";
...
/**
 * Helper function to flip an associative array, so that the value becomes the key and all the keys for that value
 * become an array of values for the new key.
 *
 * @param $array
 *  The associative array to flip.
 * @return array
 *  The flipped associative array.
 */
function _assoc_array_flip($array) {
  $flipped = array();
